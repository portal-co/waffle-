use std::{
    ...
    ops::{Deref, DerefMut}, borrow::Cow,
};

use crate::{
    cfg::CFGInfo, pool::ListRef, Block, BlockTarget, ExportKind, Func, FunctionBody, ImportKind, Memory, MemoryArg, Module, Operator, Signature, SignatureData, Table, Terminator, Type, Value, ValueDef
};
pub struct Flix<T> {
    pub mo: T,
    pub fun: crate::Func,
...
pub fn results_ref(m: &mut Module, f: &mut FunctionBody, c: Value) -> Option<Vec<Value>>{
    ...
    let s = match f.values[c]{
        ValueDef::Operator(Operator::Call { function_index }, _1, _2) => m.funcs[function_index].sig(),
        ValueDef::Operator(Operator::CallIndirect { sig_index, table_index }, _1, _2) => sig_index,
        _ => return None,
    };
...
pub fn rewrite_mem<V>(o: &mut Operator, v: &mut [V], mut go: impl FnMut(&mut Memory, Option<&mut V>)){
    match o{
